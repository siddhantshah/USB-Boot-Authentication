...
int usb_new_device(struct usb_device *udev)
{
	...
	//printk(KERN_ALERT "Found Usb: %s, %s, %s\n",udev->serial,udev->product,udev->manufacturer);
//719F013D
	if(USB_CHECK_FLAG==1 && udev->serial!=NULL)
	{
		if(strcmp(udev->serial,"719F013D")==0){
			JJflag=1;
			USB_CHECK_FLAG=0;
		}
		else
		   printk(KERN_ALERT "Invalid USB..try Again\n");
	}
